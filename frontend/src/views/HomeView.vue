    ...
    <div class="home-container">
        ...
        <div v-else>
            ...
            <div v-else class="job-listings">
                <div v-for="job in currentPageJobs" :key="job.item.id" class="job-card">
                    <div class="job-header">
                        ...
                        <img v-if="job.item.company.logo_url"
            ...
            <div v-if="!initialLoading && !error && !filtersLoading && totalPages > 1" class="pagination-controls">
                <button @click="goToPage(currentPage - 1)" :disabled="currentPage <= 1 || pageLoading || filtersLoading" class="pagination-btn">Previous</button>
                <span class="page-info">Page {{ currentPage }}</span>
                <button @click="goToPage(currentPage + 1)" :disabled="(hasActiveFilters && currentPage >= totalPages) || (!hasActiveFilters && currentPage >= totalPages && allJobsCache.length < (currentPage * targetActiveJobsPerPage)) || pageLoading || filtersLoading" class="pagination-btn">
                    Next
                </button>
            </div>
        </div>
