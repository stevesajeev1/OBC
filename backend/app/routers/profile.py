...
def get_own_profile(current_user: Annotated[DBUser, Depends(get_user)]):
    ...
    if not profile_data:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Profile cannot be found")
    return profile_data
...
def update_own_profile(
    ...
    current_user: Annotated[DBUser, Depends(get_user)]
):
...
def get_public_profile(username: str):
    ...
    if not profile:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Profile could not be found")
    return profile